...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.ASPP = ASPP(dim_in=48, dim_out=48, rate=1, bn_mom=0.1)    
    ...
    def forward(self, x):
        ...
        x_dcn = torch.cat([x0, x1, x2, x3], 1)
        aspp_out = self.ASPP(x[0])
        x = torch.cat([x_dcn, aspp_out], 1)
    ...
    def init_weights(self, pretrained='',):
        ...
        for name, m in self.named_modules():
            if any(part in name for part in {'DCN', 'Deconv','ASPP'}):
                # print('skipped', name)
                continue
